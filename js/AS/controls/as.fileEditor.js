var fe = {
    ...
    init: function (options) {
        fe.options = $.extend(fe.options, options);
        if (fe.options.loadSysLib) {
            as.sys.init();
        }
        $(document).delegate('.feBtn', 'click', function (e) {
            e.preventDefault();
            fe.showFileEditorDialog($(this));
        });
        $(document).delegate('.feDirLink', 'click', function (e) {
            e.preventDefault();
            fe.loadDir($(this).attr('data-name'));
        });
        ...
        $(document).delegate('.feFile', 'mouseenter mouseleave', function (event) {
            if (event.type === 'mouseenter') {
                ...
                if (ext == ".png" || ext == ".gif" || ext == ".jpg" || ext == ".jpeg") {
                    ...
                    $(this).append("<a href='" + url + "' class='feZoom'><img height:45px !important src='/Content/fe/zoom.png' title='" + as.resources.fileEditor_init_increasTitle + "' /></a>");
                } else {
        ...
        $(document).delegate('.feOpen', 'click', function (e) {
            ...
            as.sys.ajaxSend(fe.options.ajax.getTextFile, { url: url }, function (data) {
                //data = eval('(' + data + ')');
                ...
                if (cm && cm.sys) {
                    ...
                    fe.fileElement = cm.sys.init(el, mode);
                }
            });
        });
        ...
        $(document).delegate('.feEditFileBtn', 'click', function (e) {
            ...
            if (cm && cm.sys) {
                text = cm.sys.getValue(fe.fileElement);
            }
            var params = { url: url, text: text };
            // console.log(params);
            as.sys.ajaxSend(fe.options.ajax.saveTextFile, params, function (data) {
                //data = eval('(' + data + ')');
        ...
        $(document).delegate('.feAddFolder', 'click', function (e) {
            ...
            if (newDir) {
                ...
                as.sys.ajaxSend(fe.options.ajax.createDir, { dir: newDirFull, shared: cont.attr('data-shared'), type: cont.attr('data-type'), id: cont.attr('data-id') }, function (data) {
                    //data = eval('(' + data + ')');
        ...
        $(document).delegate(".feRenameFile", "click", function(e) {
            ...
            as.sys.showDialog(as.resources.fileEditor_init_ShowDialogRename1 + fileName, s.join(""), as.resources.fileEditor_init_ShowDialogRename2, function () {
                ...
                var params = {
                    ...
                    id:fe.options.folderID,// cont.attr('data-id'),
                    newName: newName
                };
                
               
                as.sys.ajaxSend(fe.options.ajax.renameFile, params, function (data) {
                    //data = eval('(' + data + ')');
                    if (data.result) {
                        fe.loadDir(dirName);
        ...
        $(document).delegate(".feRenameDir", "click", function(e) {
            ...
            as.sys.showDialog(as.resources.fileEditor_init_ShowDialogRenameDir1 + shortName, s.join(""), as.resources.fileEditor_init_ShowDialogRenameDir2, function () {
                ...
                as.sys.ajaxSend(fe.options.ajax.renameDir, params, function (data) {
                    //data = eval('(' + data + ')');
                    if (!data.result) {
    ...
    setUpload: function () {
        $('#feUpload').attr('data-url', fe.options.ajax.uploadFiles).fileupload({
            dataType: 'json',
                ...
                fe.loadDir(fe.curDirectory);
            },
            error: function () {
                var s = [];
                s.push("<h4>" + as.resources.fileEditor_setUpload_Error + "</h4>");
                as.sys.bootstrapAlert(s.join(""), { type: "error" });
            }
            ...
            var cont = $('.feManager');
            data.formData = { dir: fe.curDirectory, shared: cont.attr('data-shared'), type: cont.attr('data-type'), name: cont.attr('data-name'), id: cont.attr("data-id") };
        });
    },
    showFileEditorDialog: function (el, callback, options) {
        ...
        if (el) {
            $(el[0].attributes).each(function () {
                if (this.nodeName.startsWith("data-")) {
                    fe.curElementData[this.nodeName] = this.nodeValue;
                }
            });
        }
        
        fe.options.folderID = options.id;
        console.log({ shared: options.folderShared, type: options.folderType, name: options.folderName, id: options.id });
        as.sys.ajaxSend(fe.options.ajax.loadFileEditor, { shared: options.folderShared, type: options.folderType, id: options.id }, function (data) {
            //data = eval('(' + data + ')');
            var s = [];
            s.push("<div class='feManager' data-shared='" + (options.folderShared || "0") + "' data-type='" + (options.folderType || "") +
                "' data-name='" + (options.folderName || "") + "' data-id='" + options.id + "' >");
            s.push("<div class='feUpload well well-sm'>" +
                ...
                "</div>" +
                "<input id='feUpload' type='file' name='files[]' data-url='' multiple>" +
            "</div>");
            ...
            as.sys.showDialog(options.title || as.resources.fileEditor_showFileEditorDialog_showDialogFile, s.join(""), options.buttonText || as.resources.fileEditor_showFileEditorDialog_showDialogSelectFile, function () {
                ...
                var title = $('.feSelFile').attr('data-name');
                var alt = $('.feAlt').val();

                if (callback) callback(url, fe.curElementData, { alt: alt, title:title, width: selFile.attr('w'), height: selFile.attr('w') });
               // as.sys.closeDialog($('#asModal'));

            }, true, function() {
                $('.feDirsSwitch').css("background-image", "url(/Content/images/plus.png)");
                $('.feDirsSwitch').click(function (e) {
                    var im = $(e.target).css("background-image");
                    if (im.indexOf("plus") >= 0) {
                        $(e.target).css("background-image", "url(/Content/images/minus.png)");
    ...
    loadDir: function (dir) {
        ...
        console.log($('.feSelDir').closest('ul').children('li').length);
        

        var cont = $('.feManager');
        as.sys.ajaxSend(fe.options.ajax.loadDir, { dir: dir, shared: cont.attr('data-shared'), type: cont.attr('data-type'), id: cont.attr('data-id') }, function (data) {
            //data = eval('(' + data + ')');
            var s = [];
    ...
    deleteSelectedFile: function () {
        ...
        var cont = $('.feManager');
        as.sys.ajaxSend(fe.options.ajax.deleteFile, { dir: dirName, file: fileName, shared: cont.attr('data-shared'), type: cont.attr('data-type'), id: cont.attr('data-id') }, function (data) {
           // data = eval('(' + data + ')');
            if (!data.result) {
    ...
    deleteSelectedDir: function () {
        ...
        as.sys.ajaxSend(fe.options.ajax.deleteDir, { dir: dirName, shared: cont.attr('data-shared'), type: cont.attr('data-type'), id: cont.attr('data-id') }, function (data) {
            //data = eval('(' + data + ')');
            if (!data.result) {
            ...
            $('.feSelDir').hide(100, function () {
                ...
                fe.loadDir($('.feSelDir').attr('data-name'));
            });
